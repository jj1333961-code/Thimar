...
const memoryTables: Map<string, Map<string, any>> = new Map()
...
export interface Task {
  ...
  subject_id?: string
  task_type?: 'memorization' | 'review' | 'recitation' | 'homework' | 'exam' | 'general'
  surah?: number
  ayah_from?: number
  ayah_to?: number
  due_date?: string
...
export interface Evaluation {
  ...
  evaluation_type?: 'recitation' | 'memorization' | 'tajweed' | 'behavior' | 'exam' | 'general'
  score?: number
  max_score?: number
  rating?: string
  feedback?: string
...
export async function checkDatabaseConnection(): Promise<{ connected: boolean; error?: string; mode: 'supabase' | 'supabase_needs_migration' | 'local'; tables?: Record<string, boolean> }> {
  const supabase = createSupabaseAdmin()
  if (!supabase) {
  ...
  try {
    ...
    tables.app_snapshots = !snapErr

    const { error: studentErr } = await supabase.from('students').select('id').limit(1)
    tables.students = !studentErr

    const { error: taskErr } = await supabase.from('tasks').select('id').limit(1)
    tables.tasks = !taskErr

    const { error: evalErr } = await supabase.from('evaluations').select('id').limit(1)
    tables.evaluations = !evalErr

    const hasAnyTable = Object.values(tables).some(Boolean)
    if (!hasAnyTable && (snapErr?.message?.includes('does not exist') || studentErr?.message?.includes('does not exist'))) {
      return { connected: true, mode: 'supabase_needs_migration', tables }
    }

    return { connected: true, mode: 'supabase', tables }
  } catch (error) {
    return {
      connected: false,
      mode: 'local',
...
// ============================================================
// Tasks Operations (المهام والتكليفات)
// ============================================================
...
export const tasksDb = {
  async getAll(): Promise<Task[]> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      return Array.from(getMemoryTable('tasks').values()) as Task[]
    }
    try {
      const { data, error } = await supabase
        .from('tasks')
      ...
      handleError(error, 'جلب المهام')
    }
  },

  async getByStudent(studentId: string): Promise<Task[]> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const all = Array.from(getMemoryTable('tasks').values()) as Task[]
    ...
    try {
      ...
      handleError(error, 'جلب مهام الطالب')
    }
  },
  ...
  async getById(id: string): Promise<Task | null> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      return (getMemoryTable('tasks').get(id) as Task) || null
    }
    try {
      const { data, error } = await supabase
        .from('tasks')
  ...
  async create(task: Omit<Task, 'id' | 'created_at' | 'updated_at'>): Promise<Task> {
    ...
    if (!supabase) {
      const now = new Date().toISOString()
      const row: Task = { ...task, id: generateId(), created_at: now, updated_at: now }
      getMemoryTable('tasks').set(row.id, row)
      return row
    }
    try {
      const { data, error } = await supabase
      ...
      handleError(error, 'إنشاء المهمة')
    }
  },
  ...
  async update(id: string, updates: Partial<Task>): Promise<Task> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const table = getMemoryTable('tasks')
      const existing = table.get(id)
      if (!existing) throw new Error('Task not found')
      const row = { ...existing, ...updates, updated_at: new Date().toISOString() } as Task
      table.set(id, row)
      return row
    }
    try {
      ...
      handleError(error, 'تحديث المهمة')
    }
  },

  async complete(id: string, studentNotes?: string): Promise<Task> {
  ...
  async delete(id: string): Promise<void> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      getMemoryTable('tasks').delete(id)
      return
    }
    try {
      const { error } = await supabase
      ...
      handleError(error, 'حذف المهمة')
    }
  }
}

// ============================================================
// Evaluations Operations (التقييمات ودرجات التسميع)
// ============================================================
...
export const evaluationsDb = {
  async getAll(): Promise<Evaluation[]> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      return Array.from(getMemoryTable('evaluations').values()) as Evaluation[]
    }
    try {
      const { data, error } = await supabase
        .from('evaluations')
      ...
      handleError(error, 'جلب التقييمات')
    }
  },
  ...
  async getByStudent(studentId: string): Promise<Evaluation[]> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const all = Array.from(getMemoryTable('evaluations').values()) as Evaluation[]
    ...
    try {
      ...
      handleError(error, 'جلب تقييمات الطالب')
    }
  },
  ...
  async create(evaluation: Omit<Evaluation, 'id' | 'created_at' | 'updated_at'>): Promise<Evaluation> {
    ...
    if (!supabase) {
      const now = new Date().toISOString()
      const row: Evaluation = { ...evaluation, id: generateId(), created_at: now, updated_at: now }
      getMemoryTable('evaluations').set(row.id, row)
      return row
    }
    try {
      const { data, error } = await supabase
      ...
      handleError(error, 'تسجيل التقييم')
    }
  },
  ...
  async update(id: string, updates: Partial<Evaluation>): Promise<Evaluation> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const table = getMemoryTable('evaluations')
      const existing = table.get(id)
      if (!existing) throw new Error('Evaluation not found')
      const row = { ...existing, ...updates, updated_at: new Date().toISOString() } as Evaluation
      table.set(id, row)
      return row
    }
    try {
      ...
      handleError(error, 'تحديث التقييم')
    }
  },
  ...
  async delete(id: string): Promise<void> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      getMemoryTable('evaluations').delete(id)
      return
    }
    try {
      const { error } = await supabase
      ...
      handleError(error, 'حذف التقييم')
    }
  }
}

// ============================================================
// Users & Profiles Operations (المستخدمون)
// ============================================================
...
export const usersDb = {
  async getProfile(userId: string): Promise<UserProfile | null> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      return (getMemoryTable('profiles').get(userId) as UserProfile) || null
    }
    try {
      const { data, error } = await supabase
        .from('profiles')
      ...
      handleError(error, 'جلب الملف الشخصي للمستخدم')
    }
  },
  ...
  async upsertProfile(profile: Partial<UserProfile> & { id: string }): Promise<UserProfile> {
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const table = getMemoryTable('profiles')
      const existing = table.get(profile.id) || {}
    ...
    try {
      ...
      handleError(error, 'حفظ الملف الشخصي للمستخدم')
    }
  }
}
