/**
 * طبقة التحقق المستقلة من نص القرآن
 *
 * تتحقق من:
 * - صحة اسم السورة ورقمها
 * - صحة أرقام الآيات
 * - نص الآية مقابل المصدر
 * - صحة الإجابة
 * - ارتباط السؤال بالآية الصحيحة
 *
 * لا تعتمد على نموذج الذكاء الاصطناعي — تستخدم مصدر القرآن في المشروع.
 */

// ===== قائمة السور =====
const SURAH_NAMES: Array<{ number: number; name: string;ayahCount: number }> = [
...
const SURAH_MAP = new Map(SURAH_NAMES.map(s => [s.number, s]))
const SURAH_NAME_MAP = new Map(SURAH_NAMES.map(s => [s.name, s]))
...
export interface ValidationResult {
  ...
  errors: string[]
  warnings: string[]
  surahInfo?: { number: number; name: string; ayahCount: number }
}

/**
 * التحقق الشامل من سؤال قرآني
 */
export function validateQuranQuestion(question: {
  surah?: string
  surahNumber?: number
  from?: number
  to?: number
  ...
  // 1. التحقق من السورة
  let surahInfo: { number: number; name: string; ayahCount: number } | undefined

  if (question.surahNumber) {
    surahInfo = SURAH_MAP.get(question.surahNumber)
    if (!surahInfo) {
      errors.push(`رقم السورة ${question.surahNumber} غير موجود`)
    }
  } else if (question.surah) {
    surahInfo = SURAH_NAME_MAP.get(question.surah)
    if (!surahInfo) {
      errors.push(`اسم السورة "${question.surah}" غير موجود`)
    }
  } else {
    errors.push('لم يتم تحديد السورة')
  }

  // 2. التحقق من أرقام الآيات
  if (surahInfo) {
    const from = question.from || 1
    const to = question.to || from

    if (from < 1) errors.push(`رقم الآية الأولى (${from}) أقل من 1`)
    if (from > surahInfo.ayahCount) {
      errors.push(`الآية الأولى (${from}) تتجاوز عدد آيات السورة (${surahInfo.ayahCount})`)
    }
    if (to < from) errors.push(`الآية الأخيرة (${to}) أقل من الأولى (${from})`)
    if (to > surahInfo.ayahCount) {
      errors.push(`الآية الأخيرة (${to}) تتجاوز عدد آيات السورة (${surahInfo.ayahCount})`)
    }
  }

  // 3. التحقق من نوع السؤال
  const validTypes = ['mcq', 'truefalse', 'complete', 'audio']
  if (question.type && !validTypes.includes(question.type)) {
    errors.push(`نوع السؤال "${question.type}" غير صالح`)
  }

  // 4. التحقق من الإجابة
  if (question.type === 'mcq') {
    if (!question.correct) errors.push('السؤال من نوع اختيار متعدد لكن لا توجد إجابة صحيحة')
    if (question.options && question.options.length < 2) {
      errors.push('خيارات الاختيار يجب أن تكون 2 على الأقل')
    }
    if (question.correct && question.options && !question.options.includes(question.correct)) {
      errors.push('الإجابة الصحيحة غير موجودة ضمن الخيارات')
    }
  }

  if (question.type === 'truefalse') {
    if (question.correct && !['صح', 'خطأ'].includes(question.correct)) {
      errors.push('إجابة صح/خطأ يجب أن تكون "صح" أو "خطأ"')
    }
  }

  // 5. التحقق من أن السؤال لا يكشف الإجابة
  if (question.prompt && question.correct) {
    const promptLower = question.prompt.toLowerCase()
    const correctLower = question.correct.toLowerCase()
    if (correctLower.length > 5 && promptLower.includes(correctLower)) {
      warnings.push('السؤال يحتوي على الإجابة الصحيحة في النص')
    }
  }
...
export function getSurahInfo(surahNumber: number) {
  return SURAH_MAP.get(surahNumber)
}
...
export function getSurahByName(name: string) {
  return SURAH_NAME_MAP.get(name)
}
...
/**
 * التحقق من أن النص يحتوي على آية قرآنية (مقارنة تقريبية)
 */
export function containsQuranText(text: string, knownTexts: string[]): boolean {
  const normalized = text.replace(/\s+/g, ' ').trim()
  for (const known of knownTexts) {
    const normalizedKnown = known.replace(/\s+/g, ' ').trim()
