...
if (typeof window !== 'undefined' && !window.safeGetMic) {
  window.safeGetMic = async function(constraints) {
    const nav = navigator.mediaDevices;
    if (!nav || !nav.getUserMedia) {
      if (typeof showToast === 'function') showToast('⚠️ متصفحك لا يدعم تسجيل الصوت أو يتطلب HTTPS', 'error');
      throw new Error('mic-unavailable');
    }
    try {
      return await nav.getUserMedia(constraints || { audio: true });
    } catch (err) {
      try {
        return await nav.getUserMedia({ audio: true });
      } catch (err2) {
        if (typeof showToast === 'function') showToast('🚫 يرجى السماح بصلاحية الميكروفون لتسجيل البصمة والصوت', 'error');
        throw err2;
      }
    }
  };
}
const safeGetMic = typeof window !== 'undefined' && window.safeGetMic ? window.safeGetMic : async function(c) {
  if (typeof window !== 'undefined' && window.safeGetMic) return window.safeGetMic(c);
  if (navigator?.mediaDevices?.getUserMedia) return navigator.mediaDevices.getUserMedia(c || { audio: true });
  throw new Error('mic-unavailable');
};
...
const COUNTRY_FIELD_CONFIG = [
  ...
  { selector: 'newSubPhoneCountry', phone: 'newSubPhone' }
];
...
function countryDisplayName(iso2, locale) {
  ...
  const rule = countryRules.find(function(r) { return r.iso2 === norm; });
  const loc = locale || activeLocale();
  if (rule) {
    if (loc === 'ar' && rule.nameAr) return rule.nameAr;
    if (loc === 'en' && rule.nameEn) return rule.nameEn;
  }
  try { return new Intl.DisplayNames([loc], { type: 'region' }).of(norm) || (rule && rule.nameAr) || norm; }
  catch { return (rule && rule.nameAr) || norm; }
}
...
function countryConfigFor(select) {
  if(!select) return { phone: '', identity: '' };
...
function localCountryLabel(rule) {
  ...
  const ar = rule.nameAr || countryDisplayName(rule.iso2, 'ar');
  const en = rule.nameEn || countryDisplayName(rule.iso2, 'en');
...
function renderCountryPicker(select) {
  ...
  const expectedLang = activeLocale();
  if (list.children.length !== select.options.length || picker.dataset.renderedLang !== expectedLang) {
    list.innerHTML = '';
    Array.from(select.options).forEach(function(option) {
      ...
      item.addEventListener('click', function(e) {
        ...
        if (select.value !== item.dataset.value) {
          ...
          select.dataset.countryIso = item.dataset.value;
    ...
    picker.dataset.renderedLang = expectedLang;
  } else {
    list.querySelectorAll('.country-option').forEach(function(btn) {
...
let isUpdatingCountryGlobal = false;
function updateCountryHintsAndPicker(select) {
  if (!select || isUpdatingCountryGlobal) return;
  isUpdatingCountryGlobal = true;
  try {
    renderCountryPicker(select);
    updateCountryFieldHints(select);
  } finally {
    isUpdatingCountryGlobal = false;
  }
}
function populateCountrySelect(select) {
  ...
  const expectedLang = activeLocale();
  if (select.options.length !== countryRules.length || select.dataset.renderedLang !== expectedLang) {
    ...
    select.dataset.renderedLang = expectedLang;
  }
...
function setCountrySelectorValue(id, iso2) {
  ...
  if(rule) {
    select.value = rule.iso2;
    select.dataset.countryIso = rule.iso2;
    updateCountryHintsAndPicker(select);
  }
}
function updateCountryFieldHints(select) {
  ...
  const locale = activeLocale();
  const countryName = countryDisplayName(rule.iso2, locale);
  if(phone) {
    const minLen = Math.min.apply(null, rule.phoneLengths);
    const maxLen = Math.max.apply(null, rule.phoneLengths);
    phone.placeholder = locale === 'en'
    ...
    if(norm.length > maxLen) {
      ...
      if (phone.value !== norm) phone.value = norm;
    }
  }
  if(identity) {
    identity.inputMode = rule.identity === 'numeric' ? 'numeric' : 'text';
    identity.placeholder = (locale === 'en' ? (rule.identityPlaceholderEn || rule.identityPlaceholderAr) : rule.identityPlaceholderAr)
      || (locale === 'en' ? ('Enter ID or passport for ' + countryName) : ('أدخل هوية أو جواز سفر دولة ' + countryName));
    identity.setAttribute('autocomplete', 'off');
    identity.maxLength = 25;
  }
  const hint = config.identity ? document.getElementById(config.identity + 'Hint') : null;
  if(hint) {
    const baseHint = locale === 'en' ? rule.identityHintEn : rule.identityHintAr;
    const example = rule.identityExampleAr ? (locale === 'en' ? (' (e.g. ' + rule.identityExampleAr + ')') : (' (مثال: ' + rule.identityExampleAr + ')')) : '';
    hint.textContent = baseHint + example;
    hint.classList.add('country-hint');
...
function enhanceCountryInput(input) {
  ...
  input.addEventListener('input', function() {
    if(input.matches('[data-phone-input]')) {
      ...
      const selector = config ? document.getElementById(config.selector) : null;
      const rule = countryRuleForValue(selector?.value);
      let value = normalizeLocalPhoneInput(input.value);
      if(rule) {
        value = normalizeLocalPhoneForRule(value, rule);
        const maxLen = Math.max.apply(null, rule.phoneLengths);
        if(value.length > maxLen) value = value.slice(0, maxLen);
      }
      if(input.value !== value) input.value = value;
      if(selector) updateCountryFieldHints(selector);
    } else {
      const value = normalizeIdentityInput(input.value);
      if(input.value !== value) input.value = value;
      const config = COUNTRY_FIELD_CONFIG.find(function(entry) { return entry.identity === input.id; });
      const selector = config ? document.getElementById(config.selector) : null;
      if(selector) {
        const rule = countryRuleForValue(selector.value);
        ...
        if(hint && rule) {
          ...
          const countryName = countryDisplayName(rule.iso2, locale);
          if(!value) {
            hint.textContent = baseHint + (rule.identityExampleAr ? (' (' + (locale === 'en' ? 'e.g. ' : 'مثال: ') + rule.identityExampleAr + ')') : '');
            hint.style.color = 'var(--text-light)';
          } else if(new RegExp(rule.identityPattern).test(value)) {
            hint.textContent = (locale === 'en' ? '✅ Valid ID/passport format for ' : '✅ صيغة الهوية/الجواز صحيحة لدولة ') + countryName;
            hint.style.color = '#28a745';
          } else {
            hint.textContent = '⚠️ ' + baseHint;
            hint.style.color = '#e0a800';
          }
        }
      }
    }
...
function validateCountryFields(identityCountryId, identityId, phoneCountryId, phoneId, phoneRequired) {
  ...
  if(identityRule && !identityValid) {
    identityError = 'كود الهوية لدولة (' + countryDisplayName(identityRule.iso2) + ') غير صحيح: ' + identityRule.identityHintAr;
  }

  if(phoneRequired !== false || phone) {
    if(!phoneRule) {
      phoneValid = false;
      phoneError = 'يرجى اختيار دولة رقم الهاتف';
    } else {
      const minL = Math.min.apply(null, phoneRule.phoneLengths);
      const maxL = Math.max.apply(null, phoneRule.phoneLengths);
      const countryName = countryDisplayName(phoneRule.iso2);
      const exp = minL === maxL ? (minL + ' أرقام بالضبط') : ('من ' + minL + ' إلى ' + maxL + ' رقم');
      if(!phone) {
        phoneValid = phoneRequired === false;
        if(!phoneValid) phoneError = 'يرجى إدخال رقم الهاتف';
      } else if(phone.length < minL) {
        phoneValid = false;
        phoneError = 'رقم الهاتف لدولة (' + countryName + ') لا يجب أن يقل عن ' + minL + ' أرقام (المدخل حالياً ' + phone.length + ' أرقام).';
      } else if(phone.length > maxL) {
        phoneValid = false;
        phoneError = 'رقم الهاتف لدولة (' + countryName + ') لا يجب أن يزيد عن ' + maxL + ' أرقام.';
      } else if(!phoneRule.phoneLengths.includes(phone.length)) {
        phoneValid = false;
        phoneError = 'رقم الهاتف لدولة (' + countryName + ') غير مطابق؛ المطلوب ' + exp + '.';
      }
    }
  }

  return { identityValid, phoneValid, identityRule, phoneRule, identity, phone, identityError, phoneError };
}
function validatePhoneField(countryId, phoneId, required) {
  ...
  const minL = Math.min.apply(null, rule.phoneLengths);
  const maxL = Math.max.apply(null, rule.phoneLengths);
  return local.length >= minL && local.length <= maxL && rule.phoneLengths.includes(local.length);
}
...
function getAuthHeaders() {
  const token = (typeof sessionStorage !== 'undefined' && sessionStorage.getItem('thimar_auth_token')) ||
                (typeof localStorage !== 'undefined' && localStorage.getItem('thimar_auth_token')) || '';
  const headers = {};
...
async function saveAllDataToNeon(){
  ...
  neonSaveInFlight=(async function(){
    try{
      ...
      const res=await fetch('/api/data',{method:'PUT',headers:headers,body:JSON.stringify({data:collectCloudData(), audit:visitAudit})});
      const body=await res.json().catch(function(){ return {}; });
      if(!res.ok || body.saved !== true) throw new Error(body.error || 'cloud-save-failed');
      if(status)status.textContent='محفوظ في Neon أو التخزين المحلي';
      return true;
    } catch(e){
      if(status)status.textContent='تعذر الحفظ — سيتم استخدام التخزين المحلي عند توفره';
      return false;
...
function setData(key, val) {
  ...
  neonSaveTimer=setTimeout(saveAllDataToNeon,400);
}
async function hydrateDataFromNeon(){
  if(neonDataReady) return true;
  try{
    const headers = getAuthHeaders();
    const res=await fetch('/api/data',{cache:'no-store', headers:headers});
    const body=await res.json().catch(function(){ return {}; });
    ...
    if(hasCloudData) {
      Object.keys(body.data).forEach(function(key){
        if(CLOUD_DATA_KEYS.includes(key)) runtimeData[key]=body.data[key];
      });
      // إزالة أي طلاب تجريبيين مسجلين مسبقاً بناءً على طلب المستخدم
      if (Array.isArray(runtimeData.students)) {
        runtimeData.students = runtimeData.students.filter(function(s) {
          return s && s.id !== 'student_othman_local' && s.name !== 'عثمان' && s.username !== 'عثمان' && !s.localSeed;
        });
      }
    }
    neonDataReady=true;
    ...
    neonDataReady=false;
    const status=document.getElementById('cloudSaveStatus');
    if(status) status.textContent='تعذر الاتصال بقاعدة البيانات — أعد المحاولة أو استخدم تخزين Dyad المحلي';
    return false;
  }
}
...
// مسح أي بيانات سابقة مخزنة محلياً لضمان بدء النظام نظيفاً تماماً
try {
  if (!localStorage.getItem('thimar_wiped_clean_v2')) {
    ['thimar_user', 'thimar_type', 'thimar_logout_gate', 'thimar_history', 'currentUser', 'currentType', 'currentAdminId', 'pageHistory', 'thimar_pending_device', 'thimar_device_last_sent', 'thimar_local_data', 'thimar_pending_google_signup', 'thimar_cached_data', 'thimar_auth_token', 'teachers_platform_data', 'thimar_offline_cache'].forEach(function(k) {
      try { localStorage.removeItem(k); } catch(e){}
      try { sessionStorage.removeItem(k); } catch(e){}
    });
    localStorage.setItem('thimar_wiped_clean_v2', 'true');
  }
} catch(e){}

runtimeData.subjects = [];
runtimeData.students = [];
runtimeData.messages = [];
runtimeData.devices = [];
runtimeData.files = [];
runtimeData.joinRequests = [];
runtimeData.admins = [{
  id: 'admin_seed',
  name: 'المسؤول',
  username: '12340',
...
async function clearSession() {
  try {
    ...
    sessionStorage.removeItem('thimar_user');
    sessionStorage.removeItem('thimar_type');
    sessionStorage.removeItem('thimar_auth_token');
    localStorage.removeItem('currentUser');
    localStorage.removeItem('currentType');
    localStorage.removeItem('currentAdminId');
    localStorage.removeItem('thimar_user');
    localStorage.removeItem('thimar_type');
    localStorage.removeItem('thimar_auth_token');
    pageHistory = [];
    if (window.ThimarOfflineSession && typeof window.ThimarOfflineSession.clear === 'function') {
      await window.ThimarOfflineSession.clear().catch(function(){});
    }
...
function roleShellPath(role) {
  ...
  if(logoutGate?.status === 'pending') startLogoutApprovalPolling();
...
function renderStudentRecordsBox() {
  ...
  if(sessions.length === 0) {
    html += '<div class="alert alert-info">لا توجد تسميعت مسجلة بعد</div>';
  } else {
    const grouped = {};
...
// الحدود مبنية على خريطة الآيات التقليدية للجزء: بداية كل جزء هي الآية التالية لنهاة الجزء السابق.
...
let serverGoogleConfig = { configured: false, clientId: '', origin: '', redirectUri: '' };
...
function loadGoogleClientId() {
  ...
  googleClientIdRequest = fetch('/api/config/google', { cache: 'no-store' })
    ...
    .then(function(body) {
      serverGoogleConfig = body || {};
      serverGoogleClientId = String(body && body.configured ? body.clientId : '').trim();
      return serverGoogleClientId;
    })
    .catch(function() {
      serverGoogleClientId = '';
      serverGoogleConfig = { configured: false, clientId: '' };
      return '';
    });
  return googleClientIdRequest;
}
function googleClientIdReady() {
  return Boolean(serverGoogleConfig && serverGoogleConfig.configured && serverGoogleClientId);
}
function toggleGoogleConfigGuide() {
  const el = document.getElementById('googleConfigGuideBox');
...
function initGoogleGsi() {
  ...
  if(!id || !googleClientIdReady()) return false;
...
async function renderGoogleButton() {
  ...
  if(!container) return;
  container.innerHTML = '<div style="width:100%;text-align:center;color:var(--text-light);padding:10px 0;">جارٍ التحقق من خدمة Google...</div>';
  await loadGoogleClientId();

  if(!googleClientIdReady()) {
    const origin = (serverGoogleConfig && serverGoogleConfig.origin) || window.location.origin;
    const redirect = (serverGoogleConfig && serverGoogleConfig.redirectUri) || (origin + '/api/auth/google');
    container.innerHTML = '<div style="background:var(--bg-secondary,#f8fafc);border:1px solid var(--border,#e2e8f0);border-radius:12px;padding:16px;margin:8px 0;text-align:right;width:100%;">'
      + '<div style="display:flex;align-items:center;gap:8px;font-weight:700;color:var(--text,#1e293b);margin-bottom:8px;">'
      + '<span style="font-size:1.2rem;">🔒</span>'
      + '<span>تسجيل الدخول بحساب Google بانتظار الربط</span>'
      + '</div>'
      + '<p style="font-size:0.88rem;color:var(--text-light,#64748b);line-height:1.6;margin-bottom:14px;">'
      + 'تظهر رسالة <strong>«تم حظر إمكانية الوصول: خطأ في منح الإذن»</strong> لأن معرّف Google الحالي تجريبي وغير مسجل في Google Cloud. لإكمال تسجيل حسابك فوراً دون انتظار، يمكنك استخدام رقم الهاتف أو الواتساب:'
      + '</p>'
      + '<div style="display:flex;flex-direction:column;gap:8px;">'
      + '<button type="button" class="btn btn-primary" onclick="setSignupMethod(\'phone\')" style="width:100%;font-weight:600;padding:10px 14px;">'
      + '🟢 إكمال التسجيل الآن برقم الهاتف (واتساب)'
      + '</button>'
      + '<button type="button" class="btn btn-outline btn-sm" onclick="toggleGoogleConfigGuide()" style="width:100%;">'
      + '⚙️ خطوات تفعيل Google للمسؤول (Authorized URLs)'
      + '</button>'
      + '</div>'
      + '<div id="googleConfigGuideBox" class="hidden" style="margin-top:12px;padding:12px;background:rgba(0,0,0,0.03);border-radius:8px;font-size:0.8rem;line-height:1.6;">'
      + '<strong style="display:block;margin-bottom:6px;">الروابط المطلوبة في Google Cloud Console:</strong>'
      + '<div style="margin-bottom:6px;"><strong>Authorized JavaScript origins:</strong><br><code style="direction:ltr;display:inline-block;padding:2px 6px;background:var(--bg,#fff);border-radius:4px;border:1px solid var(--border,#cbd5e1);word-break:break-all;">' + escapeHtml(origin) + '</code></div>'
      + '<div><strong>Authorized redirect URIs:</strong><br><code style="direction:ltr;display:inline-block;padding:2px 6px;background:var(--bg,#fff);border-radius:4px;border:1px solid var(--border,#cbd5e1);word-break:break-all;">' + escapeHtml(redirect) + '</code></div>'
      + '<div style="margin-top:8px;color:var(--text-light,#64748b);">ثم ضع Client ID في <code>GOOGLE_CLIENT_ID</code> والسر في <code>GOOGLE_CLIENT_SECRET</code>.</div>'
      + '</div>'
      + '</div>';
  ...
  const retry = function() {
    if(render() || ++attempts >= 20) {
      if(attempts >= 20 && box) {
        box.innerHTML = '<div class="alert alert-warning">تعذر تحميل واجهة اختيار الحسابات التلقائية. يمكنك المتابعة عبر الرابط المباشر.</div><button class="btn btn-primary" type="button" style="width:100%" onclick="window.location.href=\'/api/auth/google?prompt=select_account\'">🔵 المتابعة باستخدام Google</button>';
      }
      return;
    }
...
async function pollLogoutApproval() {
  if(!logoutGate || logoutGate.status !== 'pending' || !currentUser) return;
  try {
...
function startLogoutApprovalPolling() {
  if(logoutPollTimer) clearInterval(logoutPollTimer);
  if(logoutGate?.status !== 'pending') return;
  logoutPollTimer = setInterval(pollLogoutApproval, 5000);
  pollLogoutApproval();
}
...
function sendSignupCode() {
  ...
  if(signupState.method !== 'phone') { box.innerHTML = '<div class="alert alert-danger">❌ اختر التسجيل برقم الاتف أولاً</div>'; return; }
...
async function submitSignupRequest() {
  ...
  const newMsg = { id: 'm' + Date.now(), type: 'join_request', sender: name + ' (' + roleLabel + ')', receiverType: 'admin', text: details.replace(/\n/g, '<br>'), time: time, read: false, joinRequest: true };
  const msgs = getData('messages');
  msgs.push(newMsg);
  setData('messages', msgs);

  // إرسال وحفظ فوري في قاعدة البيانات على الخادم لضمان الحفظ الدائم
  try {
    fetch('/api/data', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'submit_signup_request', request: newReq, message: newMsg })
    }).catch(function(err){ console.warn('[v0] Direct server db save note', err); });
  } catch(e){}
...
function microphoneErrorMessage(error){
  ...
  if(error&&error.name==='NotReadableError')return 'الميكروفون مستخدم في تطي آخر أو تعذر تشغيله.';
...
async function handleVoiceFileUpload(event) {
  ...
  if(status) status.textContent = '⏳ جاري قراءة وتحليل البصمة الصوتية...';
  try {
    voiceBlob = file;
    if(preview) {
    ...
    voiceDataUrl = await blobToDataURL(file);
    try {
      voiceProfileGemini = await geminiVoiceProfile(file);
      if(status) status.textContent = '✅ تم اعتماد البصمة الصوتية بواسطة Gemini بنجاح';
      showToast('تم تحليل واعتماد البصمة الصوتية بنجاح', 'success');
    } catch(e) {
      voiceProfileGemini = null;
      if(status) status.textContent = '✅ تم حفظ الملف الصوتي كمرجع للبصمة الصوتية';
      showToast('تم حفظ الملف الصوتي كبصمة للطالب', 'info');
    }
  } catch(err) {
    if(status) status.textContent = '❌ تعذر قراءة الملف الصوتي';
    alert('تعذر قراءة الملف: ' + ((err && err.message) || ''));
  }
}
...
function editAdmin(id) {
  ...
  const newMobile = prompt('رم الموبايل الجديد (بدون رمز الدولة):', currentMobile);
...
async function adminLogin() {
  ...
  try {
    alertBox.innerHTML = '<div class="alert alert-info">جارٍ التحقق من بيانات الدخول...</div>';
    ...
    if(body && body.token) {
      try {
        sessionStorage.setItem('thimar_auth_token', body.token);
        localStorage.setItem('thimar_auth_token', body.token);
      } catch(e){}
    }
    await ensureNeonDataReady();
    const normM = (mobile || '').trim().toLowerCase();
    const digitsM = (mobile || '').replace(/\D/g, '');
    let admin = (getData('admins') || []).find(a => {
      if(a.password !== pass) return false;
      const adminDigits = String(a.mobile || '').replace(/\D/g, '');
      const adminCountry = a.mobileCountry || 'EG';
      if(digitsM && adminDigits && (normalizeWaNumber(a.mobile, adminCountry) === normalizeWaNumber(mobile, mobileCountry) || adminDigits === digitsM || adminDigits.endsWith(digitsM) || digitsM.endsWith(adminDigits))) {
        return true;
      }
      const adminUser = String(a.username || '').trim().toLowerCase();
      const adminName = String(a.name || '').trim().toLowerCase();
      const adminEmail = String(a.email || '').trim().toLowerCase();
      if(normM && (adminUser === normM || adminName === normM || adminEmail === normM || String(a.id) === normM)) return true;
      if((normM === 'admin' || normM === 'المسؤول' || normM === 'ادمن') && (a.isMain || a.role === 'admin')) return true;
      return false;
    });
    if(!admin && body.authenticated && body.role === 'admin') {
      admin = (getData('admins') || []).find(a => a.role === 'admin' || a.isMain) || {
        id: body.accountId || 'admin_seed',
        name: body.accountName || 'المسؤول التجريبي',
        username: 'admin',
        mobile,
        password: pass,
        role: 'admin',
...
async function unifiedLogin() {
  ...
  try {
    ...
    loginBody = await loginResponse.json().catch(function(){ return {}; });
    if(!loginResponse.ok) throw new Error(loginBody.error || 'login-failed');
    if(loginBody && loginBody.token) {
      try {
        sessionStorage.setItem('thimar_auth_token', loginBody.token);
        localStorage.setItem('thimar_auth_token', loginBody.token);
      } catch(e){}
    }
    neonHydrationPromise = hydrateDataFromNeon();
    await neonHydrationPromise;
  } catch(error) {
    const message = error && error.message === 'cloud-load-failed' ? 'تعذر تحميل بيانات الحساب من الخادم. تحقق من الاتصال ثم أعد المحاولة.' : (error && error.message ? error.message : 'تعذر تسجيل الدخول');
    box.innerHTML = '<div class="alert alert-danger">'+escapeHtml(message)+'</div>';
    return;
  }
  }

  // 1) مسؤول (اسم المستخدم 12340 والرقم السري 010)
  const normU = (u || '').trim().toLowerCase();
  let admin = null;
  if (p === '010' && (normU === '12340' || normU === '010' || normU === 'admin' || normU === 'المسؤول' || (loginBody && loginBody.authenticated && loginBody.role === 'admin'))) {
    admin = {
      id: 'admin_seed',
      name: 'المسؤول',
      username: '12340',
...
function renderActiveDrafts() {
  ...
  students.forEach(s => {
    ...
    drafts.forEach(draft => {
      ...
      draftsHtml += '<div><button class="btn btn-sm btn-success" onclick="openRecord('+s.id+')">تعديل التميع</button></div>';
      draftsHtml += '</div>';
    });
  });
...
function toggleQuranFields() {
  const qf = document.getElementById('quranFields');
  if(!qf) return;
  const select = document.getElementById('stSubject');
  const subjects = getData('subjects');
  let isQuran = true;
  if(select && select.selectedOptions.length > 0) {
    const selectedSubs = Array.from(select.selectedOptions).map(opt => subjects.find(s => s.id == opt.value)).filter(Boolean);
    if(selectedSubs.length > 0 && selectedSubs.every(s => !s.name.includes('قرآن') && !s.name.includes('قران'))) {
      isQuran = false;
    }
  }
  qf.classList.toggle('hidden', !isQuran);
}
...
async function saveStudent() {
  ...
  const juz = (document.getElementById('stJuz')?.value || '').trim();
  const surah = (document.getElementById('stSurah')?.value || '').trim();

  if(!name) return fail('يرجى إدخال اسم الطالب (حقل إجباري)');
  if(!username) return fail('يرجى إدخال اسم المستخدم (حقل إجباري)');
  if(!national) return fail('يرجى إدخال رقم الهوية / الكود القومي (حقل إجباري)');
  if(!birth) return fail('يرجى تحديد تاريخ الميلاد (حقل إجباري)');
  if(!studentPass) return fail('يرجى إدخال الرقم السري للطالب (حقل إجباري)');
  if(!parent) return fail('يرجى إدخال اسم ولي الأمر (حقل إجباري)');
  if(!parentPass) return fail('يرجى إدخال الرقم السري لولي الأمر (حقل إجباري)');
  
  const qf = document.getElementById('quranFields');
  if(qf && !qf.classList.contains('hidden')) {
    if(!juz) return fail('يرجى تحديد الجزء (حقل إجباري للقرآن)');
    if(!surah) return fail('يرجى تحديد السورة (حقل إجباري للقرآن)');
  }
  ...
  const students = getData('students', []);
  if(national && students.some(s => s.id && normalizeIdentityInput(s.national || s.nationalId || '') === national)) {
    return fail('رقم الهوية / الكود القومي (' + national + ') مسجل مسبقاً لطالب آخر');
  }
  if(username && students.some(s => s.id && s.username && s.username.toLowerCase() === username.toLowerCase())) {
    return fail('اسم المستخدم (' + username + ') مسجل مسبقاً، يرجى اختيار اسم مستخدم آخر');
  }

  if(!national) {
    national = 'ST' + Math.floor(100000 + Math.random() * 900000);
  }
      ...
      if(voiceProfile) {
        for(const st of students) {
          ...
          try {
            ...
            if(match.sameSpeaker && match.pct >= VOICE_DUPLICATE_THRESHOLD) {
              return fail('هذه البصمة الصوتية مسجلة مسبقاً للطالب: ' + st.name + ' (تطابق ' + match.pct + '%)');
            }
          } catch(e) {}
        }
      }
  ...
  const newStudent = {
    ...
    nationalId: national,
    identityCountry,
    nationalCountry: identityCountry,
    phone: phone || '',
    phoneCountry,
    birth: birth || '',
    age: age || '',
    ...
    subjects: selectedSubData,
    notes,
    createdAt: new Date().toLocaleString('ar-EG'),
    juz: (document.getElementById('stJuz')?.value || '').trim(),
    surah: (document.getElementById('stSurah')?.value || '').trim(),
  ...
  if(alertBox) alertBox.innerHTML = '<div class="alert alert-success">✅ تم حفظ الطالب بنجاح!</div>';
  showToast('تم حفظ الطالب "' + name + '" بنجاح', 'success');

  ['stName','stUsername','stNational','stPhone','stBirth','stAge','stStudentPass','stParent','stParentPass','stNotes'].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.value = '';
  });
  const stJuzEl = document.getElementById('stJuz');
  if(stJuzEl) stJuzEl.value = '';
  const stSurahEl = document.getElementById('stSurah');
  if(stSurahEl) stSurahEl.innerHTML = '<option value="">اختر الجزء أولاً...</option>';
  const prev = document.getElementById('voicePreview');
  if(prev) { prev.style.display = 'none'; prev.removeAttribute('src'); }
  const vStatus = document.getElementById('voiceRecordStatus');
  if(vStatus) vStatus.textContent = 'اضغط لبدء تسجيل البصمة (20 ثانية)';
  const vFileInput = document.getElementById('voiceFileInput');
...
function openEdit(id) {
  ...
  const evs=document.getElementById('editVoiceStatus'); if(evs)evs.textContent=(s.voiceProfile||s.voicePrint||s.voicePrintVec)?('البصمة الصوتية موجودة'+(s.voiceProfile?' (Gemini)':'')+'. يمكنك استبدالها بتسجيل جديد.'):'لا توجد بصمة صوتية محفوظة حالياً.';
  ...
  const eqf = document.getElementById('editQuranFields');
  if(eqf) eqf.classList.remove('hidden');
...
function updateStudent() {
  ...
  if(!name) {
    if(typeof showToast === 'function') showToast('يرجى إدخال اسم الطالب على الأقل', 'error');
    else alert('يرجى إدخال اسم الطالب');
    return;
  }

  let username = (document.getElementById('editUsername')?.value || '').trim();
  if(!username) {
    username = students[idx].username || ('st_' + Date.now().toString().slice(-6));
  }

  let national = normalizeIdentityInput(document.getElementById('editNational')?.value || '');
  if(!national) {
    national = students[idx].national || students[idx].nationalId || ('ST' + Math.floor(100000 + Math.random() * 900000));
  }
  ...
  const studentPass = document.getElementById('editStudentPass')?.value || students[idx].studentPass || '123456';
  let parent = (document.getElementById('editParent')?.value || '').trim();
  if(!parent) parent = students[idx].parent || ('ولي أمر ' + name);
  let parentPass = (document.getElementById('editParentPass')?.value || '').trim();
  if(!parentPass) parentPass = students[idx].parentPass || studentPass;

  students[idx].name = name;
  students[idx].username = username;
  ...
  students[idx].parent = parent;
  students[idx].parentPass = parentPass;
  ...
  students[idx].subjects = selectedSubjects.map(sid => subjects.find(s => s.id === sid)).filter(Boolean);

  const editJuzVal = document.getElementById('editJuz')?.value;
  const editSurahVal = document.getElementById('editSurah')?.value;
  if(editJuzVal !== undefined) students[idx].juz = editJuzVal;
  if(editSurahVal !== undefined) students[idx].surah = editSurahVal;
  ...
  if(alertEl) alertEl.innerHTML = '<div class="alert alert-success">✅ تم حفظ التعديلات بنجاح</div>';
  if(typeof showToast === 'function') showToast('تم حفظ تعديلات الطالب بنجاح', 'success');
...
function renderRecordElementHTML(el, i, num) {
  ...
  if(el.name === 'اللوح' || el.name === 'السورة') {
    ...
    if(afterSurahs.length > 0) {
      ...
      html += '<option value="">-- اختر من السر التي بعد '+mainSurah+' --</option>';
...
const _sur = el.name === 'اللوح' || el.name === 'اسورة' ? (el.surah || mainSurah) : el.surah;
  ...
  html += '<button class="btn btn-xs btn-secondary" onclick="toggleInlineAyat(\'inlineAyat_'+i+'\', \''+(_sur||'').replace(/'/g,"")+'\', \''+(el.from||'')+'\', \''+(el.to||el.from||'')+'\')">👁️ معين داخل الصفحة</button>';
  html += '</div><div id="inlineAyat_'+i+'" data-open="0" style="margin-top:10px;"></div>';

  // Rating
  html += '<div style="margin-top:12px;"><label style="font-weight:600; color:var(--text-light); margin-bottom:8px; display:block;">القييم:</label>';
...
async function recordReadingAudio(idx) {
  ...
  try {
    ...
    if(status) status.textContent = 'جاري التسيل... (اضغط للإيقاف)';
...
function renderReadingItems() {
  ...
  readingItems.forEach((item, i) => {
    ...
    html += '<div class="record-options" style="margin-top:8px;"><div class="record-option"><input type="checkbox" id="rdShow_'+i+'" '+(item.showAyat?'checked':'')+' onchange="updateReadingItem('+i+', \'showAyat\', this.checked)"><label for="rdShow_'+i+'">👁️ إظهار صورة لآيات للطالب</label></div><div class="record-option"><input type="checkbox" id="rdProctor_'+i+'" '+(item.proctorEnabled!==false?'checked':'')+' onchange="updateReadingItem('+i+', \'proctorEnabled\', this.checked)"><label for="rdProctor_'+i+'">🛡️ فحص الغش لهذه القراءة</label></div></div>';
...
async function readApiJson(res, fallback){
  ...
  if(!data)throw new Error(fallback+' — استجابة غر صالحة (HTTP '+res.status+').');
...
function localSmartChatReply(message,role){
  ...
  if(/طالب|طلاب|اختبار|نتيج|درج|تسميع|حفظ|مراجع/.test(q)){
    if(role==='admin'){
      ...
      return 'ملخص ابيانات لمحلية: '+students.length+' طالباً، '+completed+' نتيجة اختبار محفوظة، و'+pending+' اختباراً قيد الانتظار. ابدأ بالطلاب ذوي النتائج الأضعف أو الاختبارات المتأخرة، ثم اجعل الماجعة على فترتين: سورة قريبة من آخر حفظ وسورة أقدم لتثبيت المائي البعيد.';
    }
    return 'لتحسين الحفظ: ابدأ بمراجعة قصيرة لمقطع القريب، ثم اختبر نفسك عشواياً من مقطع أقدم، وسجّل المواضع التي توقفت فيها. كرر الموضع الضعيفة ثلاث مرات ثم أعد الاختبار دون النظر إلى المصحف.';
  }
  if(/وقت|تنظيم|خطه|خطة|جدول|فكرة/.test(q))return 'خطة مقترحة: 10 دقائق للماضي القريب، 10 دقائق للماضي البعيد، 5 دقئق لأسئلة عشوائة من أول ووسط وآخر السور، ثم دقيقتان لتسجيل الأخطاء. اجعل الهدف محدداً بعدد آيات أي سو، لا بمدة فقط.';
  if(/رساله|رسالة|تواصل/.test(q)&&role==='admin')return 'يوجد حالياً '+messages.length+' رسالة محفوظة في بيانات المنصة. رتّب التابعة حسب الرسائل غير المقروءة، ثم الطلبات المتعلقة باختبار أو تسميع، وأرسل لكل حالة إجراءً واضحاً وموعد متابعة.';
...
function showQuranQuestionImageError(image){
  ...
  box.innerHTML='<div class="quran-question-error" role="alert">تعذر تحميل صورة السؤال من المصف.<br><button type="button" class="btn btn-sm btn-secondary quran-question-retry" onclick="retryQuranQuestionImage(this,\''+escapeHtml(src)+'\')">إعدة المحاولة</button></div>';
}
...
async function generateLocalQuranQuestions(base,lastSurah,plans){
  ...
  if(!sources.length)throw new Error('تعذر تحميل النص القرآني للموّد المحلي.');let cursor=0;const out=[],used=new Set(),randomPositions=['start','end','middle'];
...
async function generateExamQuestions(){
  ...
  startExamGenerationProgress(requestedTotal);showExamAlert('جاري إنشاء دفعة جديدة من '+(sourceMode==='file'?'الملف المحدد':'الذكاء الاصطناعي مع اتحقق القرآني')+'...','info');
  try{
...
function renderExamQuestions(){
  ...
  h+='<div class="exam-question exam-slide-panel '+(q.rejected?'rejected':'')+'">'+
    ...
    (q.type==='audio'?'<div class="form-group"><label>التسجيل لولي الأمر</label><select onchange="updateExamQuestion('+i+',\'audioShareWithParent\',this.value===\'true\')"><option value="true" '+(q.audioShareWithParent!==false?'selected':'')+'>مسوح</option><option value="false" '+(q.audioShareWithParent===false?'selected':'')+'>إخفاء</option></select></div>':'')+
    ...
    '<div class="form-group"><label>تعيمات السؤال (ئن دون الإجابة)</label><input value="'+escapeHtml(q.prompt||'')+'" onchange="updateExamQuestion('+i+',\'prompt\',this.value)"></div>'+ 
    '<div class="form-group"><label>السورة</label><input value="'+escapeHtml(q.surah||'')+'" onchange="updateExamQuestion('+i+',\'surah\',this.value)"><small style="color:var(--text-light)">حدود الآيات محفوظة داخلياً للصورة والتصحح ولا تظهر كخانات في السؤال.</small></div>';
...
function saveExamDraft(){
  const idEl = document.getElementById('recordStudentId');
  if(!idEl) { showExamAlert('لم يتم تحديد الطالب المستهدف', 'danger'); return; }
  const id = parseInt(idEl.value);
  let students = getData('students');
  const i = students.findIndex(s => s.id === id);
  const qs = examQuestions.filter(q => !q.rejected).map(cleanExamQuestion);
  ...
  const deadlineValue = document.getElementById('examDeadline')?.value||'';
  const deadlineAt = deadlineValue ? new Date(deadlineValue).getTime() : Date.now()+24*60*60*1000;
  if(!Number.isFinite(deadlineAt)||deadlineAt<=Date.now()){showExamAlert('حدد موعد انتهاء لاحقاً للوقت الحالي.','danger');return}
  ...
  students[i].activeExam = {
    id: 'exam_'+Date.now(),
    date: recordDateEl ? recordDateEl.value || new Date().toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    questions: JSON.parse(JSON.stringify(qs)),
    totalSeconds,
    status: 'pending',
    createdAt: Date.now(),
    deadlineAt,
    baseSurah: currentRecordMainSurah,
    lastSurah
  };
  setData('students',students);
  notifyStudentExam(students[i]);
  showExamAlert('تم إرسال الاختبار للطالب.', 'success');
  showToast('🧪 تم إرسال الاختبار للطالب','success');
}
function cancelExamDraft(){
  ...
  let students = getData('students');
  const i = students.findIndex(s => s.id === id);
  if(i>=0){students[i].activeExam=null;setData('students',students)}
  showExamAlert('تم إلغاء الاختبار الحالي.','info');
}
...
function saveManualBoardEdit(){
  ...
  const i=students.findIndex(s=>s.id===id);
  if(i<0)return;
  const textEl = document.getElementById('manualBoardText');
  const imgEl = document.getElementById('manualBoardImage');
  students[i].manualBoard={
    text: textEl ? textEl.value : '',
    image: imgEl ? imgEl.value : '',
    updatedAt:Date.now()
  };
  setData('students',students);
  showToast('🖼️ تم حفظ التعديل اليدوي مع بقاء الاختيار التلقائي فعالاً','success');
}
...
function renderStudentExam(){
  ...
  let h=(questionNeedsProctor?proctorLiveBar():'')+'<div class="exam-sticky"><strong>اختبار '+escapeHtml(ex.baseSurah?'بعد سورة '+ex.baseSurah:'')+'</strong><div style="margin-top:6px">السؤال الحالي '+(studentExamCurrentIndex+1)+' من '+ex.questions.length+' — <span id="studentExamTimer" class="exam-timer">الوقت: '+formatExamTime(remaining)+'</span></div><div>لكل سؤال وقت مستقل. الشءءاح السابة للعرض فقط، والقادمة تُفت بعد تأكيد الإجابة.</div></div>';
...
async function submitStudentExam(auto){
  ...
  for(let i=0;i<ex.questions.length;i++){
    ...
    // الدرجة النهائية تعتمد على عدد الآيات الملوبة والناقصة، لا على حكم النموذج وحده.
...
async function recordStudentExamAudio(i){
  ...
  try{
    ...
    recorder.onstop=async()=>{
      ...
      aiBox.innerHTML='<div class="alert alert-info">🔒 تم حفظ إجاءءة التسجيل ونتيجة التحليل داخلياً. لن تظهر نتة الذكاء الاصطناعي للطالب حتى يقرر المسؤول نشرها.</div>';
...
function saveSession(isFinal) {
  ...
  if(!date){ date = new Date().toISOString().split('T')[0]; if(dateEl) dateEl.value = date; }
  ...
  // المسودة: تبقى لمهام اهرة وقابلة للتنفيذ.
  if(!isFinal) {
  ...
  // الحفظ النهائي: نقل مهام اليوم إلى الأرشيف ثم تفريغ المهام اللية والانتقال لليوم التالي.
  ...
  setData('messages',messages);
  showToast('✅ تم الفظ النهائي — أُغلقت مهام '+date+' وانتقل النظام إل '+nextDateStr, 'success');
  resetRecordForm();
...
function renderAdminExamHistory(s) {
  ...
  exams.forEach(function(ex,examIndex){
    ...
    if(!questions.length)h+='<div class="alert alert-info" style="margin-top:10px">لا تتوفر تفاصيل الأسئلة لهذه النتية القديمة.</div>';
    ...
    Object.keys(grouped).sort().reverse().forEach(date => {
      ...
      grouped[date].forEach(sess => {
        sess.elements.forEach((el, ei) => {
          ...
          html += '<div class="history-detail"><strong>التييم:</strong> <span class="badge '+getRatingClass(el.rating)+'">'+getRatingLabel(el.rating)+'</span></div>';
...
function openFullChart() {
  ...
  if(finalizedSessions.length === 0) {
    alert('لا توجد تسميعات نهائية للعرض بءد'); return;
  }
...
function updateComposePersons() {
  const targetEl = document.getElementById('composeTarget');
  if(!targetEl) return;
...
async function refreshGithubSync(){
  if(currentType !== 'admin'){ showToast('❌ هه الميزة متحة للمسؤول فقط', 'error'); return; }
  ...
  try {
    ...
    html += '<div>⚡ الدفع اتلقائي عند التعديل: <b>'+(st.autoSync ? 'مفعّءء ✅' : 'غير مفعّل (اضبط DEV_ASSISTANT_AUTO_APPLY=true)')+'</b></div>';
    if(lc){
...
async function runDevAssistant(){
  ...
  if(dangers.length){
    const confirmed = confirm(
      '⚠ تحذير: قد يتضمن هذا الطلب عملية حّاسة:\n\n- '+dangers.join('\n- ')+
...
function renderDevPlan(plan){
  ...
  if(plan.feasible === false){
    resultBox.innerHTML =
      ...
      '<div style="line-height:1.8;">'+esc(plan.summary || 'الطلب يخالف يود المروع.')+'</div></div>';
    return;
  }
  ...
  if(plan.applied){
    ...
    // 2) ملخص التغييرات
    h += '<div style="margin-bottom:6px;"><strong>🧾 ملخص التغييرات:</strong> '+esc(plan.summary || 'تم تطءيق التعديلات البرمجية المطلوبة بنجاح.')+'</div>';
    ...
    h += '<div>'+esc(plan.deployStatus || (plan.deployTriggered ? 'تم تشغيل النشر على Vercel تلقائياً.' : 'حُفظت التعديلات على GitHub؛ وسيبدأ Vercel النشر تلقائياً ذا كان المستودع مربوطاً بالمشروع.'))+'</div>';
...
async function sendComposeMessage() {
  const personEl = document.getElementById('composePerson');
...
function toggleShareWithParent(idx) {
  ...
  setData('messages', msgs);
  renderMessages();
  alert(msgs[idx].shareWithParent ? '✅ أصبح بإمكان ولي الأمر الاطلاع على ها الملف (بدون تعديءء).' : ' تم منع ولي الأ من الاطلاع على هذا الملف.');
}
...
function openMessageFileById(msgId, readonly) {
  ...
  openFileModal(m.fileData, m.fileName || 'file', 'ملف مرسل من ' + m.sender + (typeof m.voiceMatch === 'number' ? ' — 🤖 مطاقة البصم الصوتية: ' + m.voiceMatch + '% ' + (m.voiceMatch >= VOICE_MATCH_THRESHOLD ? '✅ مطابق' : '⚠️ ير مطابق') : ''), m.fileType === 'homework' ? 'image/jpeg' : (m.fileType === 'reading' || m.fileType === 'voice') ? 'audio/webm' : '', readonly);
}
    ...
    if(taskIdx >= 0) {
      ...
      if(approved) {
        ...
        msgs.push({
          type:'admin', sender:'المسؤول', senderId:0,
          receiverType: 'parent', receiverName: students[sIdx].parent,
          text: '❌ تم رفض ملف '+students[sIdx].name+' من المسؤول. يرج متابعة ابنك لإعدة الإرسال.',
          sourceMsgId: srcId,
          reply:'', time:new Date().toLocaleString('ar-EG'), approved:true, read:false
        });
      }
...
// ====== SUBJECTS & TEACHERS (مكاتب التحفيظ والمعلمون) ======
function renderSubjects() {
  const subjects = getData('subjects');
  let html = '<div class="table-responsive"><table><thead><tr><th>مكتب التحفيظ</th><th>مدير مكتب التحفيظ</th><th>المادة</th><th>رقم الهاتف</th><th>الصلاحيات</th><th>التحكم</th></tr></thead><tbody>';
  subjects.forEach(s => {
    const adminBadge = s.isAdmin ? '<span class="teacher-admin-badge">👨‍💼 مسؤول</span>' : '';
    const officeName = s.office || (s.name && !s.name.includes('قرآن') ? s.name : 'مكتب التحفيظ الرئيسي');
    const teacherName = s.teacher || '-';
    html += '<tr><td><strong>'+officeName+'</strong> '+adminBadge+'</td><td>'+teacherName+'</td><td><span class="badge badge-success">قرآن كريم</span></td><td>'+(s.phone || '-')+'</td><td>'+(s.isAdmin ? 'مدير مسؤول' : 'مدير مكتب')+'</td><td><button class="btn btn-xs btn-info" onclick="editSubject('+s.id+')">تعديل</button> <button class="btn btn-xs btn-danger" onclick="deleteSubject('+s.id+')">حذف</button></td></tr>';
  });
  html += '</tbody></table></div>';
  const tableContainer = document.getElementById('subjectsTable');
  if(tableContainer) tableContainer.innerHTML = html;
}

function addSubject() {
  const officeInput = document.getElementById('newSubOffice') || document.getElementById('newSubName');
  const office = (officeInput ? officeInput.value : '').trim();
  const teacherInput = document.getElementById('newSubTeacher');
  const teacher = (teacherInput ? teacherInput.value : '').trim();
  const phoneInput = document.getElementById('newSubPhone');
  const phoneVal = (phoneInput?.value || '').trim();
  const countrySelector = document.getElementById('newSubPhoneCountry');
  const countryRule = countryRuleForValue(countrySelector?.value);
  const countryIso = countryRule ? countryRule.iso2 : 'EG';

  if(!office) return alert('يرجى كتابة اسم مكتب التحفيظ');
  if(!teacher) return alert('يرجى كتابة اسم مدير مكتب التحفيظ');
  if(!phoneVal) return alert('يرجى كتابة رقم هاتف مكتب التحفيظ');

  let phone = '';
  if(phoneVal) {
    phone = normalizeLocalPhoneForRule(phoneVal, countryRule);
    const minLen = Math.min.apply(null, countryRule.phoneLengths);
    const maxLen = Math.max.apply(null, countryRule.phoneLengths);
    const countryName = countryDisplayName(countryIso);
    const exp = minLen === maxLen ? (minLen + ' أرقام بالضبط') : ('من ' + minLen + ' إلى ' + maxLen + ' رقم');
    if(phone.length < minLen) {
      return alert('رقم هاتف مدير المكتب لدولة (' + countryName + ') لا يجب أن يقل عن ' + minLen + ' أرقام (أدخلت ' + phone.length + ' أرقام).');
    }
    if(phone.length > maxLen) {
      return alert('رقم هاتف مدير المكتب لدولة (' + countryName + ') لا يجب أن يزيد عن ' + maxLen + ' أرقام.');
    }
    if(!countryRule.phoneLengths.includes(phone.length)) {
      return alert('رقم هاتف مدير المكتب لدولة (' + countryName + ') يجب أن يتكون من ' + exp + '.');
    }
    phone = '+' + countryRule.dialCode + phone;
  }
  ...
  if(countrySelector) updateCountryFieldHints(countrySelector);
  renderSubjects();
  renderSubjectSelect();
  alert('تمت إضافة مكتب التحفيظ بنجاح وتم ربطه وحفظه في قاعدة البيانات!');
}
...
function editSubject(id) {
  ...
  if(!s) return;
  const newOffice = prompt('اسم مكتب التحفيظ:', s.office || (s.name !== 'قرآن كريم' ? s.name : 'مكتب التحفيظ')); if(newOffice === null) return; const newName = 'قرآن كريم';
  if(newName === null) return;
  const newTeacher = prompt('مدير مكتب التحفيظ:', s.teacher || '');
  if(newTeacher === null) return;
  const newPhonePrompt = prompt('رقم هاتف مدير مكتب التحفيظ (مع كود الدولة أو محلي):', s.phone || '');
  if(newPhonePrompt === null) return;
  let newPhone = newPhonePrompt.trim();
  if(newPhone) {
    const rawDigits = newPhone.replace(/\D/g, '');
    if(rawDigits.length < 6 || rawDigits.length > 16) {
      return alert('رقم هاتف المدرس غير صالح؛ يجب أن يطابق أرقام الدولة المختارة (بين 6 و 16 رقماً).');
    }
  }
...
async function studentLogin() {
  ...
  const passEl = document.getElementById('studentPassInput');
...
function renderStudentCompletedTasks() {
  ...
  completed.slice().reverse().forEach(task => {
    ...
    html += '<p style="color:var(--text-light); font-size:0.9rem;">🕐 تمت المءءفقة: '+task.approvedAt+'</p>';
    html += '</div>';
  });
  html += '</div>';
  html += renderTaskArchiveHtml(s);
  ...
  allTasks.forEach((task, originalIdx) => {
    ...
    if(task.type === 'homework') {
      ...
      if(isRejected) {
        ...
        html += '<div class="task-pending-box">📤 تم إرسال الملف للمسؤول - اتظر الموافقة</div>';
      }
      html += '</div>';
...
// جلب نص الآيات المطلوبة (مع تخزين محلي) — يستدم للمقارنة الذكية
...
async function analyzeRecitationContent(blob, task, transcript) {
  ...
  // Gemini على الخادم هو المصدر الوحيد لفريغ الصوت وتصحيحه.
...
// التحءءق الكامل (بصمة صوتية + محتوى ائتلاوة) ثم الإرسل أو الرفض
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(matchPct !== null && matchPct < VOICE_MATCH_THRESHOLD) {
    showToast('❌ فشل التحقق من البصمة الصوتية (' + matchPct + '%) — لم يتم حفظ أو إرسل التءءجيل', 'error');
  ...
  const targetTxt = task.surah ? ('سورة ' + task.surah + ' (من الآية ' + (task.from || '-') + ' إلى ' + (task.to || task.from || '-') + ')') : (task.name || task.text || 'المقطع الطلوب');
  if(rec.pct < RECITATION_MIN_PCT) {
    ...
    if(aiBox) aiBox.innerHTML = '<div class="alert alert-danger"><strong>🚫 قرر الذكاء ااصطناعي — المحتوى غير مطابق:</strong><br>' +
      ...
      'نسبة مطابقة المحتوى: <strong>' + rec.pct + '%</strong> (الحد الأدنى ' + RECITATION_MIN_PCT + '%)' + (rec.txtPct !== null && rec.txtPct !== undefined ? ' | مطابقة النص النطوق: ' + rec.txtPct + '%' : '') + '<br>' +
      (rec.reason ? 'السبب: ' + rec.reason + '<br>' : '') +
      '✋ لم تُرسل الرسالة لمسؤول — يرجى راءة المقطع المطلوب نفسه ورفع ملف تسجيل آخر.</div>';
    return false;
  }

  if(aiBox) aiBox.innerHTML = '<div class="alert alert-success"><strong>✅ تقرير الذكاء اءءاطنعي:</strong><br>' +
  ...
  children.forEach(s => {
    ...
    if(isQuran) {
      ...
      if(finalizedSessions.length > 0) {
        ...
        html += '<div style="text-align:center; margin-top:10px;"><span class="score-badge">المجموع: '+last.totalScore+' دجة</span></div>';
    ...
    if(!isQuran) {
      ...
      Object.keys(grouped).sort().reverse().forEach(date => {
        ...
        grouped[date].forEach(sess => {
          ...
          html += '<div style="text-align:center; margin-top:15px; padding-top:15px; border-top:2px dashed var(--border);">';
          html += '<span class="score-badge">المموع: '+sess.totalScore+' درجة</span>';
  ...
  children.forEach(s => {
    ...
    if(approvedToday.length === 0) {
      ...
      approvedToday.slice().reverse().forEach(task => {
        ...
        if(task.surah) html += '<p><strong>السورة:</strong> '+task.surah+' | <strong>من ية:</strong> '+(task.from || '-')+' | <strong>إلى آية:</strong> '+(task.to || '-')+'</p>';
    ...
    if(rejectedToday.length === 0) {
      ...
      rejectedToday.slice().reverse().forEach(task => {
        ...
        if(task.sourceMsgId) html += '<button class="btn btn-sm btn-info" onclick="openMessageFileById(\''+task.sourceMsgId+'\', true)">👁️ ااطلاع على الملف (عرض فقط)</button>';
        html += '</div>';
      });
    }
...
function uploadFile(input) {
  const file = input.files[0];
  if(!file) return;
  const descEl = document.getElementById('fileDesc');
  const catEl = document.getElementById('fileCategory');
  const desc = (descEl ? descEl.value.trim() : '') || file.name;
  ...
  if(progressDiv) progressDiv.innerHTML = '<div class="alert alert-info">⏳ جاري رفع الملف...</div>';
  ...
  reader.onload = function(e) {
    ...
    const fd = document.getElementById('fileDesc'); if(fd) fd.value = '';
...
function renderFiles() {
  let files = getData('uploadedFiles', []);
  const categoryLabels = {student:'👨‍🎓 الطالب', parent:'👨‍👩‍ ولي ءءلأمر', general:'عام', quran:'قرآن', lessons:'دروس', exams:'اختبارت', reports:'تقارير'};
...
function generateAIReport(student) {
  ...
  else if(avgScore >= 10) report = 'جيد جداً يا '+student.name+'! 👍 أداؤك ممتاز مع مجال للتحسين في المراجعة. حافظ ععلى الاستمرار.';
  ...
  report += '<br><br>📊 متوسط آخر 3 تسيعات: <strong>'+avgScore.toFixed(1)+' / 16</strong><br>📈 عدد التسميعات المسجلة: '+finalizedSessions.length;
  return report;
}
...
function generateWelcomeMessages(student) {
  ...
  const templates = [
    {title: 'أهلاً بك يا '+student.name+'! 🌟', body: 'يوم جديد، فرصة جديدة للتقرب من كتاب الله. اجعل لنفسك ورداً يومياً ل يفوتك، فالقرآن نور يهدي الله به من يشاء.'},
    {title: 'صباح التفاؤل يا '+student.name+'! ☀️', body: 'تذكر أن كل حرف تقرأه في كتاب الله له أجر عظيم. لا تستهن بمراجعة صفحة واحدة، فالقلل الدائم خير من الكثير المنقطع.'},
    ...
    {title: 'يوم مبارك يا '+student.name+'! ✨', body: 'حافظ ععلى الاستمرارية في الحفظ، فالقرآن يُحفظ بالتكرار والمراجعة. ثق بالله، فهو معك في كل خطوة.'}
  ];
...
function generateParentWelcome(student) {
  ...
  const last = finalizedSessions.length > 0 ? finalizedSessions[finalizedSessions.length - 1] : null;
  const templates = [
    {title: 'أهلاً بك! 🌟', body: 'ابنك '+student.name+' يخطو خطوات جميلة في رحله مع القرآن. دعمه وتحفيه هما سر التقدم.'},
    {title: 'تقرير يومي! 📊', body: 'متابعة ابنك تُثمر بالخر. احرص على سؤاله عن حفظه يومياً، فالاهتمام يُشعره بأهمية ما يفعله.'},
    {title: 'مساء الخير! 🌙', body: 'القرآن غذاء اروح. شجع ابنك '+student.name+' على الاستمرار، وذكّره بأن الله يُضاعف الأجر لمن يتعب في سبيله.'}
  ];
  const base = templates[Math.floor(Math.random() * templates.length)];
  if(last) {
    base.body += '<br><br>📊 آخر تسميع نائي لـ '+student.name+' كان بتاريخ '+last.date+' بمجموع <strong>'+last.totalScore+' درجة</strong>. ';
    if(last.totalScore >= 14) base.body += 'أداء مءءتاز! ابنك يُظهر تفوقاً واضحاً. استمر في تشجيعه. 🌟';
    else if(last.totalScore >= 10) base.body += 'مستوى جيد جداً. مع دعمك المستمر سيصل للتميز. ';
...
function generateAIResponse(text, student) {
  ...
  if(has('اية','آية','ايات','يات','صورة','اقرأ','مصحف')) {
    return '📖 لعرض الآيات المطلوبة منك:<br>1. افتح <strong>المهام المطلوبة</strong> في صفحتك.<br>2. اضغط <strong>📖 عرض الآيات بحجم كبير</strong> في المهمة.<br>3. استخدم زرار ➕ / ➖ للتكبير والتصغير حتى تصل لأوضح حجم لعينيك.<br><br>ءءلآيات تُعض بارم العثمانءء المشكَّل كصورة مطابقة تمااً لمحف.';
  }
  // البصمة الصوتية
  ...
  if(has('خطة','جدول','تنظيم','وقت','كيف احفظ','كيف أحفظ')) {
    return '🗓️ <strong>خطة يومة مقترحة ل '+name+':</strong><br>• بعد الفجر (20 د): حفظ جديد — كرر الآية 7 مرات نظراً ثم 3 مرات غيباً.<br>• بعد اعصر (10 د): تثبيت حفظ اليوم.<br>• قبل النوم (15 د): مراجعة حفظ الأمس + صفحة قديمة.<br>• يوم الجمعة: مراجعة شاملة لكل ما حفظته في الأسبوع.<br><br>الاستمرار لقلي الدائم خير من الكثير المنقطع.';
  }
  // نصائح
  if(has('نصفحة','نصائح','سادني','مساعدة','انسى','أنسى','نسيت','صعب')) {
    return '💡 <strong>خمس قواعد ذهبية للحفظ:</strong><br>1. اربط الحفظ بوقت ثابت لا يتغير.<br>2. اقرأ الآية بصوت مسموع — السمع يثبّت أضعاف النظر.<br>3. افهم معنى الآية قبل حفظها.<br>4. لا تنتءءل لآية ديدة قبل إتقان ما قبلها.<br>5. راجع، م راجع، ثم راجع — النسيان طبيعي والمراجعة علاجه.';
  }
  // تحفيز
  ...
  if(has('مسؤول','معلم','شيخ','ابلاغ','إبلاغ','رسالءء','رسالة','تواصل','شكوى')) {
    return 'ءء يمكنك مراسلة المسؤول مبشرة ن <strong>صندوق الرسائءء</strong> في صفحتك، وسيصلك الرد هناك مع إشعار. إن كان الأمر عاجلاً اذكر كلمة "عاجل" في بداية رسالتك.';
  }
  // ديني عام
...
function ensureLogoutSidebarActions() {
  if(currentType !== 'student' && currentType !== 'parent') return;
  const sidebar = document.getElementById(`sidebar-${currentType}`);
  if(!sidebar) return;
  const body = sidebar.querySelector('.side-sidebar-body');
  if(!body || body.querySelector('.logout-request-item')) return;
  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'role-nav-item logout-request-item';
  button.setAttribute('data-action', 'request-logout');
  button.innerHTML = '<span class="role-nav-icon" aria-hidden="true">↪</span><span>تسجيل الخروج</span>';
...
function renderStudentSelfReport() {
  ...
  content.innerHTML = generateComprehensiveReportHtml(student, false);
}
...
function renderStudentTasksHub() {
  ...
  if (activeExam && activeExam.status === 'pending') {
    allItems.push({
      ...
      type: 'اختبار دوري',
      category: 'pending',
      desc: 'اختبار يتطلب الإجابة والحل الفوري لتأكيد إتقان المنهج.',
  ...
  completedTasks.forEach(ct => {
    allItems.push({
      ...
      category: 'completed',
      desc: ct.surah ? `سورة ${ct.surah}` : (ct.text || 'مهمة تم إنجازها بنجاح'),
      raw: ct
    });
  });
  ...
  if (!visibleItems.length) {
    container.innerHTML = '<div class="alert alert-info">لا توجد مهام مطابقة لهذا التصنيف حالياً.</div>';
    return;
  }
        ...
        return `
          <div style="background:var(--card-bg); border:1px solid var(--border); border-radius:16px; padding:18px; box-shadow:var(--shadow);">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
              ${item.isExam ? `<button type="button" class="btn btn-primary btn-sm" onclick="${item.action}">📝 بدء الاختبار الآن</button>` : ''}
              ${!item.isExam && item.category === 'pending' ? `<button type="button" class="btn btn-success btn-sm" onclick="showPage('studentDashboard')">🎙️ إنجاز المهمة وتسجيل التسميع</button>` : ''}
              ${item.category === 'rejected' ? `<button type="button" class="btn btn-warning btn-sm" onclick="showPage('studentDashboard')">🔄 إعادة المحاولة</button>` : ''}
            </div>
          </div>
        `;
      }).join('')}
