...
if(!runtimeData.initialized_v7) {
  ...
  runtimeData.admins = [{id:'admin_seed', name:'المسؤول التجريبي', mobile:'01000000000', password:'1234', role:'admin', isMain:true}];
  runtimeData.initialized_v7 = true;
  // يبدأ الحفظ بعد نجاح تسجيل الدخول فقط، لأن API يحمي بيانات Neon.
  } else {
  let admins = getData('admins');
  let changed = false;
  if(!Array.isArray(admins) || !admins.length){ admins=[{id:'admin_seed',name:'المسؤول التجريبي',mobile:'01000000000',password:'1234',role:'admin',isMain:true}]; changed=true; }
  admins.forEach(a => { if(a.mobile === '0000000000' || a.mobile === '00000000000') { a.mobile = '01000000000'; a.name = a.name || 'المسؤول التجريبي'; a.role = a.role || 'admin'; changed = true; } });
...
function renderStudentRecordsBox() {
  ...
  if(sessions.length === 0) {
    html += '<div class="alert alert-info">لا توجد تسميع��ت مسجلة بعد</div>';
  } else {
    const grouped = {};
...
function localSmartChatReply(message,role){
  ...
  if(/طالب|طلاب|اختبار|نتيج|درج|تسميع|حفظ|مراجع/.test(q)){
    if(role==='admin'){
      ...
      return 'ملخص ابيانات ��لمحلية: '+students.length+' طالباً، '+completed+' نتيجة اختبار محفوظة، و'+pending+' اختباراً قيد الانتظار. ابدأ بالطلاب ذوي النتائج الأضعف أو الاختبارات المتأخرة، ثم اجعل الم��اجعة على فترتين: سورة قريبة من آخر حفظ وسورة أقدم لتثبيت المائي البعيد.';
    }
...
function renderExamQuestions(){
  ...
  h+='<div class="exam-question exam-slide-panel '+(q.rejected?'rejected':'')+'">'+
    ...
    '<div class="form-group"><label>تع��يمات السؤال (ئن دون الإجابة)</label><input value="'+escapeHtml(q.prompt||'')+'" onchange="updateExamQuestion('+i+',\'prompt\',this.value)"></div>'+ 
...
function renderStudentExam(){
  ...
  let h=(questionNeedsProctor?proctorLiveBar():'')+'<div class="exam-sticky"><strong>اختبار '+escapeHtml(ex.baseSurah?'بعد سورة '+ex.baseSurah:'')+'</strong><div style="margin-top:6px">السؤال الحالي '+(studentExamCurrentIndex+1)+' من '+ex.questions.length+' — <span id="studentExamTimer" class="exam-timer">الوقت: '+formatExamTime(remaining)+'</span></div><div>لكل سؤال وقت مستقل. الشءءا��ح الساب��ة للعرض فقط، والقادمة تُفت�� بعد تأكيد الإجابة.</div></div>';
...
async function runDevAssistant(){
  ...
  if(dangers.length){
    const confirmed = confirm(
      '⚠��� تحذير: قد يتضمن هذا الطلب عملية ح��ّاسة:\n\n- '+dangers.join('\n- ')+
...
function openMessageFileById(msgId, readonly) {
  ...
  openFileModal(m.fileData, m.fileName || 'file', 'ملف مرسل من ' + m.sender + (typeof m.voiceMatch === 'number' ? ' — 🤖 مطا��قة البصم الصوتية: ' + m.voiceMatch + '% ' + (m.voiceMatch >= VOICE_MATCH_THRESHOLD ? '✅ مطابق' : '⚠️ ير مطابق') : ''), m.fileType === 'homework' ? 'image/jpeg' : (m.fileType === 'reading' || m.fileType === 'voice') ? 'audio/webm' : '', readonly);
}
...
async function verifyAndSubmitRecitation(taskIdx, blob, dataUrl, transcript, aiBoxId, statusEl, fileName) {
  ...
  if(matchPct !== null && matchPct < VOICE_MATCH_THRESHOLD) {
    showToast('❌ فشل التحقق من البصمة الصوتية (' + matchPct + '%) — لم يتم حفظ أو إرس��ل التءءجيل', 'error');
...
async function inspectVoiceQuality(blob){
  ...
  const ctx=new AC();try{
    ...
    if(clippedRatio>.02||peak>=.999)throw new Error('مستوى الصوت مرتفع ويسبب تشويشًا. ابتعد قليلًا عن الميكروفو�� وأعد التسجيل.');
...
async function verifyVoiceIdentity(blob,student){
  ...
  if(!profile)throw new Error('لا توجد بصمة Gemini مرجعية مفوءءة له��ا الطالءء');
  const audio=await voiceAudioPayload(blob);
...
function generateAIResponse(text, student) {
  ...
  if(has('خطة','جدول','تنظيم','وقت','كيف احفظ','كيف أحفظ')) {
    return '🗓️ <strong>خطة يوم��ة مقترحة ل�� '+name+':</strong><br>• بعد الفجر (20 د): حفظ جديد — كرر الآية 7 مرات نظراً ثم 3 مرات غيباً.<br>• بعد ا��عصر (10 د): تثبيت حفظ اليوم.<br>• قبل النوم (15 د): مراجعة حفظ الأمس + صفحة قديمة.<br>• يوم الجمعة: مراجعة شاملة لكل ما حفظته في الأسبوع.<br><br>الاستمرار لقلي�� الدائم خير من الكثير المنقطع.';
  }
  // نصائح
