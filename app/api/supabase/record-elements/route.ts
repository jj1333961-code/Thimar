...
import { recordElementsDb } from '@/lib/supabase/database'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      const all = await recordElementsDb.getAll()
...
export async function POST(request: NextRequest) {
  try {
    ...
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      const element = await recordElementsDb.create({
        student_id,
        type,
        title,
        description,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const supabase = createSupabaseAdmin()
    if (!supabase) {
      await recordElementsDb.delete(id)
